...
pub enum Method {
    ...
    /// Spark-UTK ("user-key tweak"): RGB commitment embedded in the
    /// user-side public key before FROST aggregation, surfacing on L1 as
    /// `p2tr(taproot_noscript(verifyingKey), no-script)` after a Spark
    /// unilateral exit. See [`crate::keytweak`] for the construction.
    #[display("sparkutk")]
    SparkUtk = 0x02,
}
...
impl FromStr for Method {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        Ok(match s.to_lowercase() {
            ...
            s if s == Method::SparkUtk.to_string() => Method::SparkUtk,
