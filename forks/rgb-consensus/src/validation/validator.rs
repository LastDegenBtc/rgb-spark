        ...
        Witness<Dbc>: SealWitness<BlindSeal<Txid>, Message = mpc::Commitment>,
    {
        ...
        match anchor.convolve(self.contract_id, message) {
            ...
            Ok(commitment) => {
                ...
                // Driven by proof.method() rather than inferring from the
                // first matching output — p2tr is now ambiguous between
                // TapretFirst and SparkUtk, and only the proof knows which
                // variant it is. Precise byte-level disambiguation between
                // those two p2tr-producing methods is delegated to the
                // proof's own `verify()` (called via verify_many_seals
                // below).
                let proof_method = witness.proof.method();
                let has_compatible_output = witness.tx.outputs().any(|out| match proof_method {
                    CloseMethod::OpretFirst => out.script_pubkey.is_op_return(),
                    CloseMethod::TapretFirst | CloseMethod::SparkUtk => {
