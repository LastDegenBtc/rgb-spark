...
use bp::dbc::keytweak::{SparkUtkError, SparkUtkProof};
...
pub enum DbcError {
    ...
    /// the witness transaction has no p2tr output to verify the Spark-UTK
    /// seal against.
    NoP2trOutput,
}
...
impl dbc::Proof for DbcProof {
    ...
    fn method(&self) -> Method {
        match self {
            ...
            DbcProof::SparkUtk(_) => Method::SparkUtk,
        }
    }
    ...
    fn verify(&self, msg: &Commitment, tx: &Tx) -> Result<(), Self::Error> {
        match self {
            ...
            DbcProof::SparkUtk(spark) => spark.verify(msg, tx).map_err(|err| match err {
                SparkUtkError::NoP2trOutput => DbcError::NoP2trOutput,
                SparkUtkError::OutputKeyMismatch => DbcError::CommitmentMismatch,
                SparkUtkError::InvalidOutputXonly(_) | SparkUtkError::Secp(_) => {
                    DbcError::InvalidProof
                }
            }),
        }
    }
